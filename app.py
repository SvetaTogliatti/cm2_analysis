...
# Исходные данные
data = [
    ['Skyeng English Old',122613,15.77,0.6667,0.5,0.937,51124.22,23161.03,0.0,
     0.008,62429.84,0.15,9,0.2903,0.3676,0.0853,0,0,0.0181,285,3579],
    ['Skyeng English Courses & Programmes',216119,4.27,0.7231,0.9344,0.937,116033.72,35039.72,0.0,
     0.008,130281.04,0.15,9,0.2903,0.3676,0.0853,0.00116,0.17,0.0404,5234,2937],
    ['Skysmart School',247997,9.26,0.7593,0.9359,0.937,82208.96,15174.63,0.0,
     0.008,110584.93,0.15,9,0.2903,0.3676,0.0853,0.002432,0.13,0.0191,9143,5178],
    ['Skysmart Homeschooling',21994,5.07,0.7415,0.9311,0.937,99869.91,14751,0.0,
     0.008,66940.63,0.15,9,0.2903,0.3676,0.0853,0.001712,0.25,0.0544,876,79],
    ['Skysmart Preschool',25746,9.32,0.7631,0.9535,0.937,70825.75,20140.36,0.0,
     0.008,84901.29,0.15,9,0.2903,0.3676,0.0853,0.00188,0.2,0.0368,1306,357],
    ['Skysmart Coding',62258,9.30,0.7815,0.9516,0.937,101590.19,24171.89,0.0,
     0.008,128775.13,0.15,9,0.2903,0.3676,0.0853,0.001176,0.17,0.0326,3821,862],
    ['Skysmart Exams',41348,12.10,0.7628,0.9463,0.937,91172.79,24851.56,0.0,
     0.008,98266.09,0.15,9,0.2903,0.3676,0.0853,0.002848,0.11,0.0197,2250,603],
    ['Skysmart Exams Large Classes',19021,8.79,0.8361,0.9391,0.937,50645.17,6888.04,0.0,
     0.008,31109.05,0.15,9,0.2903,0.3676,0.0853,0.001824,0.14,0.0356,896,179]
]
...
columns = ['portfolio','orders','conv','approve_rate','take_rate','CR_internal_loan',
           ...
           'bank_payments','installment_payments']
...
def calculate_financial_metrics(df, include_refunds):
    if include_refunds:
        df['cash_in_bank_loan'] = df['bank_payments'] * df['ATV_bank_loan'] * (1 - df['bank_loan_commission'] - df['ya_split_comission']) * (1 - df['bank_loan_refunds_share'])
        df['cash_in_internal'] = df['installment_payments'] * df['FTV_internal_loan'] * 0.7 * (1 - df['internal_loan_commission']) * df['inflation_adjustment_factor'] * (1 - df['internal_loan_refunds_share'])
        df['GMV'] = df['cash_in_bank_loan'] + df['cash_in_internal']
        df['CM2'] = df['GMV'] * (1 - df['sm'] - df['cogs'] - df['ops'])
        df['CM2_bank_loan'] = df['cash_in_bank_loan'] * (1 - df['sm'] - df['cogs'] - df['ops'])
        df['CM2_internal_loan'] = df['cash_in_internal'] * (1 - df['sm'] - df['cogs'] - df['ops'])
    else:
        df['cash_in_bank_loan'] = df['bank_payments'] * df['ATV_bank_loan'] * (1 - df['bank_loan_commission'] - df['ya_split_comission'])
        df['cash_in_internal'] = df['installment_payments'] * df['FTV_internal_loan'] * 0.7 * (1 - df['internal_loan_commission']) * df['inflation_adjustment_factor']
        df['GMV'] = df['cash_in_bank_loan'] + df['cash_in_internal']
        df['CM2'] = df['GMV'] * (1 - df['sm'] - df['cogs'] - df['ops'])
        df['CM2_bank_loan'] = df['cash_in_bank_loan'] * (1 - df['sm'] - df['cogs'] - df['ops'])
        df['CM2_internal_loan'] = df['cash_in_internal'] * (1 - df['sm'] - df['cogs'] - df['ops'])
...
def calculate_CM2(weights, internal_loan_repayment_rate, include_refunds=is_include_refunds):
    weight_bank_loan, weight_internal_loan = weights
...
def main():
    ...
    st.sidebar.write("- [Consolidated 24-25](https://docs.google.com/spreadsheets/d/1g3mAswh1iweDZHsmRMBM5NjkT2va7ITUmFv_wXM8hfY/edit?gid=245601635#gid=245601635)")
